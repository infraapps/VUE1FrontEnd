  ...
  <div role="tablist">
    <b-card no-body class="mb-1">
      <b-card-header header-tag="header" class="p-1" role="tab">
        <b-button block href="#" v-b-toggle.accordion-1 variant="info">Nuevo Usuario</b-button>
      </b-card-header>
        ...
        <b-card-body>
          <div class="container">
            <b-form @submit="onSubmit" @reset="onReset" v-if="show">
               ...
               <b-form-group id="input-group-7" label="Cliente:" label-for="input-8">
                <b-form-select id="input-8" v-model="form.cliente" :options="cliente" required></b-form-select>
              </b-form-group>
              <b-form-group
                id="input-group-1"
                label="Nombre completo:"
            ...
            <b-card class="mt-3" header="Resultado del formulario">
              <pre class="m-0">{{ form }}</pre>
            </b-card>
          </div>
              <div v-for="(item, index) in usuarios" :key="index"> <h1>{{item.USUARIO_ID}} Solo validar</h1></div>
...
export default {
  data() {
    ...
    url: "http://localhost:3000/posventa/sebas/usuarios.php/usuarios/",
    info: "",
      usuarios: [],
      async created() {
        try {
          const res = await axios.get(
            'http://localhost:3000/posventa/sebas/usuarios.php/usuarios/'
          );
